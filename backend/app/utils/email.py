...
def send_email_sync(msg):
    try:
        mail.send(msg)
        print(f"[EMAIL] Email sent successfully to {msg.recipients}", flush=True)
...
def send_credentials_email(full_name, email, enrollment_id, username, password, domain, plan):
    ...
    try:
        ...
        print(f'[EMAIL] Failed to send credentials email to {email}: {e}', flush=True)
...
def send_quote_request_email(name, email, phone, project_type, features, estimated_cost, description):
    try:
        msg = Message(
            ...
            recipients=['aitechpulze@gmail.com'],
            reply_to=email
        )
...
def send_student_application_admin_email(data):
    try:
        admin_email = current_app.config.get('ADMIN_EMAIL', 'info@aitechpulze.com')
        print(f"[EMAIL] Sending student application notification to {admin_email}", flush=True)
        msg = Message(
        ...
        import traceback
        print(f"[EMAIL] Failed to send student app admin email: {type(e).__name__}: {e}", flush=True)
        traceback.print_exc()
